...
use std::sync::{Mutex, Arc, Weak as ArcWeak};
use super::{Site, OriginKind, Origin, Metadata, Uid};
...
impl<T> Narc<T> {
	...
	fn clone_at_site(&self, site: Site) -> Narc<T> {
		...
		let parent_origin = map.strongs.get(&self.id)
	...
	fn downgrade_at_site(this: &Self, site: Site) -> Weak<T> {
		...
		let prev_origin = map.strongs.get(&this.id)
	...
	pub fn upgrade_at_site(&self, site: Site) -> Option<Narc<T>> {
		self.inner.upgrade().map(|inner| {
			let id = {
				...
				let prev_origin = map.weaks.get(&self.id)
